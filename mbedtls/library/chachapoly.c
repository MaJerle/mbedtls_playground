 ...
 */
static int
chachapoly_pad_aad(mbedtls_chachapoly_context* ctx) {
 ...
 */
static int
chachapoly_pad_ciphertext(mbedtls_chachapoly_context* ctx) {
...
void
mbedtls_chachapoly_init(mbedtls_chachapoly_context* ctx) {
...
int
mbedtls_chachapoly_setkey(mbedtls_chachapoly_context* ctx, const unsigned char key[32]) {
...
int
mbedtls_chachapoly_starts(mbedtls_chachapoly_context* ctx, const unsigned char nonce[12],
...
int
mbedtls_chachapoly_update_aad(mbedtls_chachapoly_context* ctx, const unsigned char* aad, size_t aad_len) {
...
int
mbedtls_chachapoly_update(mbedtls_chachapoly_context* ctx, size_t len, const unsigned char* input,
...
int
mbedtls_chachapoly_finish(mbedtls_chachapoly_context* ctx, unsigned char mac[16]) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
static int
chachapoly_crypt_and_tag(mbedtls_chachapoly_context* ctx, mbedtls_chachapoly_mode_t mode, size_t length,
...
int
mbedtls_chachapoly_encrypt_and_tag(mbedtls_chachapoly_context* ctx, size_t length, const unsigned char nonce[12],
...
int
mbedtls_chachapoly_auth_decrypt(mbedtls_chachapoly_context* ctx, size_t length, const unsigned char nonce[12],
    ...
    if ((ret = chachapoly_crypt_and_tag(ctx, MBEDTLS_CHACHAPOLY_DECRYPT, length, nonce, aad, aad_len, input, output,
                                        check_tag))
        != 0) {
        return (ret);
    }
...
int
mbedtls_chachapoly_self_test(int verbose) {
    ...
    for (i = 0U; i < 1U; i++) {
        ...
        ret = mbedtls_chachapoly_encrypt_and_tag(&ctx, test_input_len[i], test_nonce[i], test_aad[i], test_aad_len[i],
                                                 test_input[i], output, mac);
