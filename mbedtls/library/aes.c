...
mbedtls_aes_setkey_enc(mbedtls_aes_context* ctx, const unsigned char* key, unsigned int keybits) {
    ...
    switch (ctx->nr) {
        case 10:
            ...
            for (i = 0; i < 10; i++, RK += 4) {
                RK[4] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(RK[3])])
                        ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(RK[3])] << 8) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(RK[3])] << 16)
        ...
        case 12:
            ...
            for (i = 0; i < 8; i++, RK += 6) {
                RK[6] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(RK[5])])
                        ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(RK[5])] << 8) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(RK[5])] << 16)
        ...
        case 14:
            ...
            for (i = 0; i < 7; i++, RK += 8) {
                RK[8] = RK[0] ^ RCON[i] ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(RK[7])])
                        ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(RK[7])] << 8) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(RK[7])] << 16)
                ...
                RK[12] = RK[4] ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(RK[11])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(RK[11])] << 8)
...
#if defined(MBEDTLS_AESNI_C) && defined(MBEDTLS_HAVE_X86_64)
    if (mbedtls_aesni_has_support(MBEDTLS_AESNI_AES)) {
        mbedtls_aesni_inverse_key((unsigned char*)RK, (const unsigned char*)(cty.buf + cty.rk_offset), ctx->nr);
        goto exit;
    }
#endif
...
#if defined(MBEDTLS_CIPHER_MODE_XTS)
static int
mbedtls_aes_xts_decode_keys(const unsigned char* key, unsigned int keybits, const unsigned char** key1,
...
int
mbedtls_aes_xts_setkey_enc(mbedtls_aes_xts_context* ctx, const unsigned char* key, unsigned int keybits) {
...
int
mbedtls_aes_xts_setkey_dec(mbedtls_aes_xts_context* ctx, const unsigned char* key, unsigned int keybits) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    const unsigned char *key1, *key2;
    unsigned int key1bits, key2bits;
...
#define AES_FROUND(X0, X1, X2, X3, Y0, Y1, Y2, Y3)                                                                     \
    do {                                                                                                               \
        (X0) = *RK++ ^ AES_FT0(MBEDTLS_BYTE_0(Y0)) ^ AES_FT1(MBEDTLS_BYTE_1(Y1)) ^ AES_FT2(MBEDTLS_BYTE_2(Y2))         \
               ^ AES_FT3(MBEDTLS_BYTE_3(Y3));                                                                          \
                                                                                                                       \
        (X1) = *RK++ ^ AES_FT0(MBEDTLS_BYTE_0(Y1)) ^ AES_FT1(MBEDTLS_BYTE_1(Y2)) ^ AES_FT2(MBEDTLS_BYTE_2(Y3))         \
               ^ AES_FT3(MBEDTLS_BYTE_3(Y0));                                                                          \
                                                                                                                       \
        (X2) = *RK++ ^ AES_FT0(MBEDTLS_BYTE_0(Y2)) ^ AES_FT1(MBEDTLS_BYTE_1(Y3)) ^ AES_FT2(MBEDTLS_BYTE_2(Y0))         \
               ^ AES_FT3(MBEDTLS_BYTE_3(Y1));                                                                          \
                                                                                                                       \
        (X3) = *RK++ ^ AES_FT0(MBEDTLS_BYTE_0(Y3)) ^ AES_FT1(MBEDTLS_BYTE_1(Y0)) ^ AES_FT2(MBEDTLS_BYTE_2(Y1))         \
               ^ AES_FT3(MBEDTLS_BYTE_3(Y2));                                                                          \
    } while (0)

#define AES_RROUND(X0, X1, X2, X3, Y0, Y1, Y2, Y3)                                                                     \
    do {                                                                                                               \
        (X0) = *RK++ ^ AES_RT0(MBEDTLS_BYTE_0(Y0)) ^ AES_RT1(MBEDTLS_BYTE_1(Y3)) ^ AES_RT2(MBEDTLS_BYTE_2(Y2))         \
               ^ AES_RT3(MBEDTLS_BYTE_3(Y1));                                                                          \
                                                                                                                       \
        (X1) = *RK++ ^ AES_RT0(MBEDTLS_BYTE_0(Y1)) ^ AES_RT1(MBEDTLS_BYTE_1(Y0)) ^ AES_RT2(MBEDTLS_BYTE_2(Y3))         \
               ^ AES_RT3(MBEDTLS_BYTE_3(Y2));                                                                          \
                                                                                                                       \
        (X2) = *RK++ ^ AES_RT0(MBEDTLS_BYTE_0(Y2)) ^ AES_RT1(MBEDTLS_BYTE_1(Y1)) ^ AES_RT2(MBEDTLS_BYTE_2(Y0))         \
               ^ AES_RT3(MBEDTLS_BYTE_3(Y3));                                                                          \
                                                                                                                       \
        (X3) = *RK++ ^ AES_RT0(MBEDTLS_BYTE_0(Y3)) ^ AES_RT1(MBEDTLS_BYTE_1(Y2)) ^ AES_RT2(MBEDTLS_BYTE_2(Y1))         \
...
int
mbedtls_internal_aes_encrypt(mbedtls_aes_context* ctx, const unsigned char input[16], unsigned char output[16]) {
    int i;
    ...
    t.X[0] = *RK++ ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(t.Y[0])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(t.Y[1])] << 8)
             ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(t.Y[2])] << 16) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(t.Y[3])] << 24);

    t.X[1] = *RK++ ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(t.Y[1])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(t.Y[2])] << 8)
             ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(t.Y[3])] << 16) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(t.Y[0])] << 24);

    t.X[2] = *RK++ ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(t.Y[2])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(t.Y[3])] << 8)
             ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(t.Y[0])] << 16) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(t.Y[1])] << 24);

    t.X[3] = *RK++ ^ ((uint32_t)FSb[MBEDTLS_BYTE_0(t.Y[3])]) ^ ((uint32_t)FSb[MBEDTLS_BYTE_1(t.Y[0])] << 8)
             ^ ((uint32_t)FSb[MBEDTLS_BYTE_2(t.Y[1])] << 16) ^ ((uint32_t)FSb[MBEDTLS_BYTE_3(t.Y[2])] << 24);
...
int
mbedtls_internal_aes_decrypt(mbedtls_aes_context* ctx, const unsigned char input[16], unsigned char output[16]) {
    int i;
    uint32_t* RK = ctx->buf + ctx->rk_offset;
    ...
    t.X[0] = *RK++ ^ ((uint32_t)RSb[MBEDTLS_BYTE_0(t.Y[0])]) ^ ((uint32_t)RSb[MBEDTLS_BYTE_1(t.Y[3])] << 8)
             ^ ((uint32_t)RSb[MBEDTLS_BYTE_2(t.Y[2])] << 16) ^ ((uint32_t)RSb[MBEDTLS_BYTE_3(t.Y[1])] << 24);

    t.X[1] = *RK++ ^ ((uint32_t)RSb[MBEDTLS_BYTE_0(t.Y[1])]) ^ ((uint32_t)RSb[MBEDTLS_BYTE_1(t.Y[0])] << 8)
             ^ ((uint32_t)RSb[MBEDTLS_BYTE_2(t.Y[3])] << 16) ^ ((uint32_t)RSb[MBEDTLS_BYTE_3(t.Y[2])] << 24);

    t.X[2] = *RK++ ^ ((uint32_t)RSb[MBEDTLS_BYTE_0(t.Y[2])]) ^ ((uint32_t)RSb[MBEDTLS_BYTE_1(t.Y[1])] << 8)
             ^ ((uint32_t)RSb[MBEDTLS_BYTE_2(t.Y[0])] << 16) ^ ((uint32_t)RSb[MBEDTLS_BYTE_3(t.Y[3])] << 24);

    t.X[3] = *RK++ ^ ((uint32_t)RSb[MBEDTLS_BYTE_0(t.Y[3])]) ^ ((uint32_t)RSb[MBEDTLS_BYTE_1(t.Y[2])] << 8)
             ^ ((uint32_t)RSb[MBEDTLS_BYTE_2(t.Y[1])] << 16) ^ ((uint32_t)RSb[MBEDTLS_BYTE_3(t.Y[0])] << 24);
 ...
 */
static void
mbedtls_gf128mul_x_ble(unsigned char r[16], const unsigned char x[16]) {
    uint64_t a, b, ra, rb;
 ...
 */
int
mbedtls_aes_crypt_xts(mbedtls_aes_xts_context* ctx, int mode, size_t length, const unsigned char data_unit[16],
                      const unsigned char* input, unsigned char* output) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 */
int
mbedtls_aes_crypt_cfb128(mbedtls_aes_context* ctx, int mode, size_t length, size_t* iv_off, unsigned char iv[16],
                         const unsigned char* input, unsigned char* output) {
    int c;
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t n;
 ...
 */
int
mbedtls_aes_crypt_cfb8(mbedtls_aes_context* ctx, int mode, size_t length, unsigned char iv[16],
                       const unsigned char* input, unsigned char* output) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    unsigned char c;
    unsigned char ov[17];
 ...
 */
int
mbedtls_aes_crypt_ofb(mbedtls_aes_context* ctx, size_t length, size_t* iv_off, unsigned char iv[16],
                      const unsigned char* input, unsigned char* output) {
    int ret = 0;
    size_t n;
 ...
 */
int
mbedtls_aes_crypt_ctr(mbedtls_aes_context* ctx, size_t length, size_t* nc_off, unsigned char nonce_counter[16],
...
mbedtls_aes_self_test(int verbose) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            mbedtls_printf("  AES-ECB-%3u (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
        }
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            mbedtls_printf("  AES-CBC-%3u (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
        }
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            mbedtls_printf("  AES-CFB128-%3u (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
        }
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0) {
            mbedtls_printf("  AES-OFB-%3u (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
        }
