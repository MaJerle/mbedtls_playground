...
void
mbedtls_pem_init(mbedtls_pem_context* ctx) {
 ...
 */
static int
pem_get_iv(const unsigned char* s, unsigned char* iv, size_t iv_len) {
    size_t i, j, k;
...
static int
pem_pbkdf1(unsigned char* key, size_t keylen, unsigned char* iv, const unsigned char* pwd, size_t pwdlen) {
...
pem_pbkdf1(unsigned char* key, size_t keylen, unsigned char* iv, const unsigned char* pwd, size_t pwdlen) {
    ...
    if ((status = psa_hash_finish(&operation, md5sum, PSA_HASH_LENGTH(PSA_ALG_MD5), &output_length)) != PSA_SUCCESS) {
        goto exit;
    }
 ...
 */
static int
pem_aes_decrypt(unsigned char aes_iv[16], unsigned int keylen, unsigned char* buf, size_t buflen,
...
int
mbedtls_pem_read_buffer(mbedtls_pem_context* ctx, const char* header, const char* footer, const unsigned char* data,
...
void
mbedtls_pem_free(mbedtls_pem_context* ctx) {
    if (ctx->buf != NULL) {
...
int
mbedtls_pem_write_buffer(const char* header, const char* footer, const unsigned char* der_data, size_t der_len,
