 ...
 */
static inline void
chacha20_quarter_round(uint32_t state[16], size_t a, size_t b, size_t c, size_t d) {
    /* a += b; d ^= a; d <<<= 16; */
    state[a] += state[b];
    state[d] ^= state[a];
 ...
 */
static void
chacha20_block(const uint32_t initial_state[16], unsigned char keystream[64]) {
...
int
mbedtls_chacha20_setkey(mbedtls_chacha20_context* ctx, const unsigned char key[32]) {
...
int
mbedtls_chacha20_starts(mbedtls_chacha20_context* ctx, const unsigned char nonce[12], uint32_t counter) {
    /* Counter */
...
int
mbedtls_chacha20_update(mbedtls_chacha20_context* ctx, size_t size, const unsigned char* input, unsigned char* output) {
    size_t offset = 0U;
    size_t i;
...
int
mbedtls_chacha20_crypt(const unsigned char key[32], const unsigned char nonce[12], uint32_t counter, size_t data_len,
...
int
mbedtls_chacha20_self_test(int verbose) {
    ...
    for (i = 0U; i < 2U; i++) {
        ...
        ret = mbedtls_chacha20_crypt(test_keys[i], test_nonces[i], test_counters[i], test_lengths[i], test_input[i],
                                     output);
