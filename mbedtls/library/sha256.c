...
#if defined(MBEDTLS_SHA256_USE_A64_CRYPTO_IF_PRESENT) || defined(MBEDTLS_SHA256_USE_A64_CRYPTO_ONLY)
#include <arm_neon.h>
#endif
...
static int
mbedtls_a64_crypto_sha256_determine_support(void) {
...
static size_t
mbedtls_internal_sha256_process_many_a64_crypto(mbedtls_sha256_context* ctx, const uint8_t* msg, size_t len) {
...
static
    ...
    return ((mbedtls_internal_sha256_process_many_a64_crypto(ctx, data, SHA256_BLOCK_SIZE) == SHA256_BLOCK_SIZE) ? 0
                                                                                                                 : -1);
}
...
#if !defined(MBEDTLS_SHA256_PROCESS_ALT) && !defined(MBEDTLS_SHA256_USE_A64_CRYPTO_ONLY)
...
#endif
    int
    mbedtls_internal_sha256_process_c(mbedtls_sha256_context* ctx, const unsigned char data[SHA256_BLOCK_SIZE]) {
    struct {
...
static size_t
mbedtls_internal_sha256_process_many_c(mbedtls_sha256_context* ctx, const uint8_t* data, size_t len) {
    size_t processed = 0;
...
static int
mbedtls_a64_crypto_sha256_has_support(void) {
    static int done = 0;
...
static size_t
mbedtls_internal_sha256_process_many(mbedtls_sha256_context* ctx, const uint8_t* msg, size_t len) {
...
int
mbedtls_internal_sha256_process(mbedtls_sha256_context* ctx, const unsigned char data[SHA256_BLOCK_SIZE]) {
    if (mbedtls_a64_crypto_sha256_has_support()) {
 ...
 */
int
mbedtls_sha256(const unsigned char* input, size_t ilen, unsigned char* output, int is224) {
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if ((ret = mbedtls_sha256_starts(&ctx, is224)) != 0 || (ret = mbedtls_sha256_update(&ctx, input, ilen)) != 0
        || (ret = mbedtls_sha256_finish(&ctx, output)) != 0) {}
 ...
 */
int
mbedtls_sha256_self_test(int verbose) {
