...
static void
cmac_xor_block(unsigned char* output, const unsigned char* input1, const unsigned char* input2,
 ...
 */
static void
cmac_pad(unsigned char padded_block[MBEDTLS_CIPHER_BLKSIZE_MAX], size_t padded_block_len,
...
int
mbedtls_cipher_cmac(const mbedtls_cipher_info_t* cipher_info, const unsigned char* key, size_t keylen,
 ...
 */
int
mbedtls_aes_cmac_prf_128(const unsigned char* key, size_t key_length, const unsigned char* input, size_t in_len,
...
static int
cmac_test_subkeys(int verbose, const char* testname, const unsigned char* key, int keybits,
                  const unsigned char* subkeys, mbedtls_cipher_type_t cipher_type, int block_size, int num_tests) {
    int i, ret = 0;
    mbedtls_cipher_context_t ctx;
    const mbedtls_cipher_info_t* cipher_info;
    unsigned char K1[MBEDTLS_CIPHER_BLKSIZE_MAX];
    ...
    for (i = 0; i < num_tests; i++) {
        ...
        if ((ret = mbedtls_cipher_setkey(&ctx, key, keybits, MBEDTLS_ENCRYPT)) != 0) {
             ...
             * function to fail. */
            if ((ret == MBEDTLS_ERR_PLATFORM_FEATURE_UNSUPPORTED || ret == MBEDTLS_ERR_CIPHER_FEATURE_UNAVAILABLE)
...
static int
cmac_test_wth_cipher(int verbose, const char* testname, const unsigned char* key, int keybits,
...
mbedtls_cmac_self_test(int verbose) {
    ...
    if ((ret = cmac_test_wth_cipher(verbose, "AES 128", aes_128_key, 128, test_message, aes_message_lengths,
                                    ...
                                    MBEDTLS_AES_BLOCK_SIZE, NB_CMAC_TESTS_PER_KEY))
        != 0) {
        return (ret);
    }
    ...
    if ((ret = cmac_test_wth_cipher(verbose, "3DES 2 key", des3_2key_key, 192, test_message, des3_message_lengths,
                                    ...
                                    MBEDTLS_DES3_BLOCK_SIZE, NB_CMAC_TESTS_PER_KEY))
        != 0) {
        return (ret);
    }
